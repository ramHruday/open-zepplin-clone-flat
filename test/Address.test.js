const { accounts, contract } = artifacts.require(
  "@openzeppelin/test-environment"
);

const { balance, constants, ether, expectRevert, send } = artifacts.require(
  "@openzeppelin/test-helpers"
);
const { expect } = artifacts.require("chai");
...
describe("Address", function () {
  ...
  describe("toPayable", function () {
    it("should return a payable address when the account is the zero address", async function () {
      expect(await this.mock.toPayable(constants.ZERO_ADDRESS)).to.equal(
    ...
    it("should return a payable address when the account is the all ones address", async function () {
      expect(await this.mock.toPayable(ALL_ONES_ADDRESS)).to.equal(
        ALL_ONES_ADDRESS
      );
    });
  });
  ...
  describe("sendValue", function () {
    ...
    context("when sender contract has funds", function () {
      ...
      it("sends non-zero amounts", async function () {
        ...
        expect(await this.recipientTracker.delta()).to.be.bignumber.equal(
          funds.subn(1)
        );
      });
      ...
      it("sends the whole balance", async function () {
        ...
        expect(await balance.current(this.mock.address)).to.be.bignumber.equal(
          "0"
        );
      });
